...
# Базовые пользователи (добавлен параметр banned)
DEFAULT_DATA = {
    "1": {"name": "Пётр I", "balance": 1000, "pin": "1111", "banned": False, "history": [{"type": "Система", "amount": 1000, "comment": "Стартовый капитал"}]},
    "2": {"name": "Екатерина II", "balance": 1500, "pin": "2222", "banned": False, "history": [{"type": "Система", "amount": 1500, "comment": "Стартовый капитал"}]},
    ...
    "7": {"name": "—", "balance": 0, "pin": "7777", "banned": False, "history": []},
...
def load_db():
    ...
    try:
        with open(DB_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
            # Проверка, чтобы у всех старых юзеров был параметр banned
            updated = False
            for uid, uinfo in data.items():
                if "banned" not in uinfo:
                    uinfo["banned"] = False
                    updated = True
            if updated:
                save_db(data)
